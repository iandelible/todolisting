...
RSpec.describe TasksController, type: :controller do
  ...
  describe "task#create" do
    it "should allow new tasks to be created" do
      post :create, task: {title: "Understand the post verb and create action"}
      expect(response).to have_http_status :success
      response_value = ActiveSupport::JSON.decode(@response.body)
      expect(response_value['title']).to eq("Understand the post verb and create action")
      expect(Task.last.title).to eq("Understand the post verb and create action")
    end
  end
end
